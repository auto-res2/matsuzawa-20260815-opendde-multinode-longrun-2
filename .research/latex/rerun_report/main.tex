...
ジョブ全体の793秒の差について、本報告の初版では「実行イメージの準備とデータ読み込みの差」と記したが、
両ランのログを突き合わせた結果これは誤りだったため訂正する。ログで検証できる各フェーズ
──投入からコンテナ内最初のログまで（前回165秒／今回153秒）、セットアップ（チェックポイント読込と
NCCL 初期化、162秒／142秒）──はほぼ同一であり、成果物の回収（finetuned.pt のクラスタ外への
コピー、両ランとも約2分）はジョブ終了後に行われるためこの計時には含まれない。
差のほぼ全ては前回ランの学習中に局在する：前回のログはエポック3の最終ステップ（16:15:39）から
エポック4の最初のステップ（16:28:58）まで\textbf{799秒にわたり全16ランクが沈黙}しており、
その前後のステップ間隔は中央値23.2秒で一定である。奇妙な点として、前回ランが自己申告した
学習ループ時間（1411.2秒）はこの空白を\emph{除いた}時間列と一致し（$23.2\,\text{秒} \times 60$
ステップ $+$ 端数 $\approx 1411$秒）、ログのタイムスタンプ列の実測（約2,143秒）とは矛盾する。
共有ファイルシステムの一時停滞またはノード時刻の補正が疑われるが、これらのログだけでは原因を
確定できない。確定できるのは、この差がイメージ構築・データ読み込み・成果物コピーの
いずれでもないことである。
...
\subsection{件数差（14/19 対 18/24）の原因は \texttt{--limit} の違い}
\label{sec:limit}

前回報告の評価（14 assembly／19界面）と武藤氏再現の評価（18／24）の件数差は、
乱数シードでもデータの置き場でもなく、\textbf{起動方法の違いによる \texttt{--limit} の差}である。
前回報告の数値は \texttt{docker/Dockerfile.eval} の CMD 経由で実行されており、そこには
\texttt{--limit 16} がハードコードされている。実際、そのランのログ（Seyval ラン
\texttt{6844f687}、2026-08-15）には
\texttt{16 targets from interface\_antibody\_antigen} と記録されている。
一方、武藤氏再現は手書きの \texttt{infer.slurm}（29行目に \texttt{--limit 20}）で起動されており、
そのログ（\texttt{/data1/rkp00041/rku00121/mz-repro/logs/mz-repro-infer-28972.err}）には
\texttt{20 targets ...} と記録されている。

原子数超過（\texttt{--max-atoms 8000}）でスキップされたのは両者とも同一の2件
（8xnh: 15,706原子、8up2: 10,632原子）であり、$14 = 16 - 2$、$18 = 20 - 2$ で件数は完全に説明される。
前回の14件が再現の18件に丸ごと含まれるのは、同じ順序のターゲット表を \texttt{--limit} で
頭から切り詰めているためであり、「前回側で4件多く予測に失敗した」わけではない
──その4件（ターゲット表の17〜20番目）は前回の実行には最初から投入されていない。
重なる14件の値がほぼ一致する（例：8tuz の DockQ 0.018 対 0.016）のは推論シードが42に
固定されているためで、残る微差は GPU の非決定的カーネルによる丸め差である。
なお \texttt{opendde\_data} の個人コピーと共有ツリーの内容差
（共有側にのみ \texttt{obsolete\_to\_successor.json} 等が存在する）は、この件数差には寄与していない。

\subsection{リポジトリの不変性について}

本ランのチェックポイントを評価する目的で \texttt{docker/Dockerfile.eval} のチェックポイント参照を
変更するコミット（\texttt{6e01ac9}）を一度作成したが、リポジトリを実験の記録として不変に保つため
revert した（\texttt{ba89232}。履歴には両コミットが残る）。本ランの成果物
\texttt{finetuned.pt}（クラスタ上の配置先：
\texttt{/data1/rkp00041/rku00122/opendde-multinode/checkpoints/finetuned-4node-30ep.pt}）に対する
DockQ 評価は、本報告の時点でも未実施のままである。実施する場合は、リポジトリを変更しない方法
（元の \texttt{Dockerfile.eval} が読む上記の固定パスをそのまま用いるか、リポジトリ外の
起動スクリプトで \texttt{src.inference}／\texttt{src.evaluate} を直接呼ぶ）を取るべきである。
